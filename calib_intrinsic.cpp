...
//   img_dir = opendir (imgs_directory);

//   for (int k = 1; k <= 2*num_imgs; k++) {
//     img_ent = readdir(img_dir);
//     std::string img_dname(img_ent->d_name);
//     if (img_dname.find(std::string(imgs_filename))== std::string::npos) {
//       continue;
//     }
//     std::cout << "Found file " << img_ent->d_name << std::endl;
//     if (img_dname.size() < 12) {
//       std::cout << "Discarding file..." << '\n';
//       continue;
//     }
//     std::string frame = img_dname.substr(strlen(imgs_filename), 4);
//     std::string img_filename = std::string(imgs_filename)+frame+"."+std::string(extension);
//     std::string img_file = std::string(imgs_directory)+img_filename;
//     std::cout << "Opening file " << img_file << std::endl;
//     // if(!doesExist(img_file))
//     //   std::cout << "imgs dne " << img_file << std::endl;
//     //   continue;
...
void setup_calibration(int board_width, int board_height, int num_imgs, 
  ...
  for (int k = 1; k <= num_imgs; k++) {
    ...
    if(!doesExist(img_file)) {
      std::cout << "File dne " << img_file << std::endl;
      continue;
    }
